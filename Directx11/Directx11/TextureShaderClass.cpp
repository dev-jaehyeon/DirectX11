...
//Render 함수는 이제 새로운 텍스처 파라미터를 가진다. 이것은 텍스처 리소스를 가리키는 포인터다. 이것은 SetShaderParameter로 보내지고
//텍스처는 셰이더에 세팅되며 렌더링에 사용될 수 있게 된다.
...
/// <summary>
/// OutputShaderErrorMessage는 셰이더가 로드되지 않았을 때 처리하기 위함이다.
/// </summary>
...
/// <summary>
/// SetShaderParameters 함수는 이제 텍스처 자원을 가리키는 포인터를 취하고 이것을 셰이더에 할당한다. 이것은 버퍼가 렌더링되기 전에 세팅되어야 한다.
/// </summary>
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	//이전 튜토리얼과 다른 점은 이제 텍스처가 픽셀셰이더에 포함된다는 것
...
void TextureShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	//RenderShader 함수는 SampleState를 포함하도록 변경되었다.
